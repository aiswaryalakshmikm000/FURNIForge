...
export class ResendForgotPasswordOtpUseCase {
  ...
  async execute(data: ResendForgotPasswordOtpDTO): Promise<ResendOtpResponseDTO> {
    ...
    const otp = await this.otpService.generateAndHandleOtp( user.id, user.email.value );

    await this.emailService.sendOTPEmail( user.email.value, otp.otp, user.firstName );
