...
export default function AdminConfigRatesPage() {
  ...
  const [addCategory, setAddCategory] =
    useState<ConfigCategory | null>(null);

  const [editingConfigRate, setEditingConfigRate] =
    useState<ConfigRateResponseDTO | null>(null);
  ...
  const {
    mutateAsync: createConfigRate,
    isPending: isCreating,
  } = useCreateConfigRate();

  const {
    mutateAsync: updateConfigRate,
  ...
  // } = useToggleConfigRateStatus();

  // const {
  //   mutateAsync: deleteConfigRate,
